...
namespace DA_Assets.FCU
{
    ...
    public class PrefabCreator : MonoBehaviourBinder<FigmaConverterUnity>
    {
        ...
        private PrefabStruct CreatePrefabStruct(SyncHelper syncObject)
        {
            ...
            if (monoBeh.IsUGUI())
            {
                ps.UguiTransformData = new UguiTransformData(syncObject.GetComponent<RectTransform>());
            }
            else if (monoBeh.IsNova())
            {
#if NOVA_EXISTS
                ps.NovaTransformData = new NovaTransformData(syncObject.GetComponent<UIBlock>());
#endif
            }
        ...
        private void InstantiatePrefab(ref PrefabStruct ps, int i, UnityEngine.Object pref)
        {
            ...
            else
            {
                ...
                if (monoBeh.IsUGUI())
                {
                    ps.UguiTransformData.ApplyTo(ps.InstantiatedPrefab.GetComponent<RectTransform>());
                }
                else if (monoBeh.IsNova())
                {
#if NOVA_EXISTS
                    ps.NovaTransformData.ApplyTo(ps.InstantiatedPrefab.GetComponent<UIBlock>());
#endif
                }
